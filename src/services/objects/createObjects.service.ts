...
const createObjectsService = async ({typeId, userId, description, name}:ICreateObjects) => {
    if(!description || !name) throw new AppError(400, "object must have name and description");
    if(!typeId || !userId) throw new AppError(400, "object must have typeId and userId");
